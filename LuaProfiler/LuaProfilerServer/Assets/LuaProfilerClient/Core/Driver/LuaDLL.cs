...
";
        ...
        public unsafe static int luaL_loadbufferUnHook(IntPtr luaState, byte[] buff, IntPtr size, string name)
        {
        ...
        public unsafe static void BindEasyHook()
        {
            if (m_hooked) return;
            UnityEngine.Debug.Log("begin hook");
            ...
            UnityEngine.Debug.Log(LUA_VERSION);
            if (luaL_newstate_hook == null)
            {
        ...
        public static IntPtr luaL_newstate_replace()
        {
            lock (m_Lock)
            {
                ...
                UnityEngine.Debug.Log("is hooked");
                if (isHook)
                {
        ...
        public static unsafe int luaL_loadbuffer_replace(IntPtr luaState, IntPtr buff, IntPtr size, string name, IntPtr mode)
        {
            lock (m_Lock)
            {
                if (isHook)
